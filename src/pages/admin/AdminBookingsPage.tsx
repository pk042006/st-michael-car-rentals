  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by ref, name, email..." className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#c9a84c]" /></div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#c9a84c] bg-white">
          {['all','pending','confirmed','active','completed','cancelled','rejected'].map(s => <option key={s} value={s}>{s === 'all' ? 'All Statuses' : s.charAt(0).toUpperCase() + s.slice(1)}</option>)}
        </select>
      </div>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? [...Array(8)].map((_, i) => <tr key={i} className="border-b border-gray-50">{[...Array(7)].map((_, j) => <td key={j} className="px-4 py-3"><Skeleton className="h-4 w-full" /></td>)}</tr>) :
              filtered.length === 0 ? <tr><td colSpan={7} className="px-4 py-12 text-center text-gray-500">No bookings found</td></tr> :
              filtered.map(b => (
                <tr key={b.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-mono text-xs text-gray-700">{b.booking_reference}</td>
                  <td className="px-4 py-3"><div className="font-medium text-gray-900">{b.full_name}</div><div className="text-xs text-gray-400">{b.email}</div></td>
                  ...
                  <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${getStatusColor(b.status)}`}>{b.status}</span></td>
                  <td className="px-4 py-3 hidden lg:table-cell font-semibold text-gray-900">{formatCurrency(b.total || 0)}</td>
                  <td className="px-4 py-3"><Link to={`/admin/bookings/${b.id}`} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-gray-900 inline-flex" title="View details"><Eye size={16} /></Link></td>
                </tr>
              ))}
            </tbody>
          </table>
