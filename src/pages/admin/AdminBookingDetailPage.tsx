...
export function AdminBookingDetailPage() {
  ...
  if (loading) return <div className="space-y-4"><Skeleton className="h-8 w-64" /><div className="grid grid-cols-2 gap-4">{[...Array(6)].map((_, i) => <Skeleton key={i} className="h-24 rounded-2xl" />)}</div></div>
  if (!booking) return null

  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        <Link to="/admin/bookings" className="p-2 hover:bg-gray-100 rounded-xl"><ChevronLeft size={20} className="text-gray-600" /></Link>
        <div><h1 className="font-serif text-2xl font-bold text-gray-900">Booking {booking.booking_reference}</h1><span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${getStatusColor(booking.status)}`}>{booking.status}</span></div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <h2 className="font-semibold text-gray-900 mb-3">Vehicle</h2>
          {booking.vehicles && <div><img src={booking.vehicles.images?.[0] || ''} alt="" className="w-full h-32 object-cover rounded-xl mb-3" /><div className="font-bold text-gray-900">{booking.vehicles.name}</div><div className="text-sm text-gray-500">{booking.vehicles.category}</div><div className="text-lg font-bold text-[#c9a84c] mt-1">{formatCurrency(booking.vehicles.price_per_day)}/day</div></div>}
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div><span className="text-gray-500">From:</span> <div className="font-medium">{booking.pickup_location}</div></div>
            <div><span className="text-gray-500">To:</span> <div className="font-medium">{booking.return_location}</div></div>
          </div>
        </div>
      </div>
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4 text-sm">
          ...
          <div><span className="text-gray-500 block">Total</span><span className="font-bold text-xl text-[#c9a84c]">{formatCurrency(booking.total || 0)}</span></div>
        </div>
        <div className="flex items-center gap-3">
          <label className="text-sm font-medium text-gray-700">Payment Status:</label>
          <select value={paymentStatus} onChange={e => setPaymentStatus(e.target.value)} className="px-3 py-1.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#c9a84c]">{['unpaid','pending','paid','refunded'].map(s => <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>)}</select>
        </div>
      </div>
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <h2 className="font-semibold text-gray-900 mb-3">Admin Notes</h2>
        <textarea value={adminNotes} onChange={e => setAdminNotes(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#c9a84c] resize-none" rows={3} placeholder="Internal notes..." />
        {booking.notes && <div className="mt-3 p-3 bg-blue-50 rounded-xl text-sm text-blue-700"><span className="font-medium">Customer notes: </span>{booking.notes}</div>}
        <Button onClick={handleSaveNotes} loading={updating} variant="outline" size="sm" className="mt-3"><Save size={14} /> Save Notes</Button>
      </div>
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <h2 className="font-semibold text-gray-900 mb-4">Update Status</h2>
        <div className="flex flex-wrap gap-2">{STATUS_ACTIONS.filter(a => a.status !== booking.status).map(action => (<button key={action.status} onClick={() => handleStatusUpdate(action.status)} disabled={updating} className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all disabled:opacity-60 ${action.color}`}>{action.label}</button>))}</div>
      </div>
    </div>
  )
}