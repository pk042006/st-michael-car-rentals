...
import { Car, BookOpen, Users, DollarSign, TrendingUp, CheckCircle, Clock, XCircle, AlertCircle, Activity } from 'lucide-react'
import { supabase } from '../../lib/supabase'
import { Booking, Vehicle, User } from '../../types/database'
...
export function AdminDashboardPage() {
  ...
  const last14Days = Array.from({ length: 14 }, (_, i) => {
    ...
    const count = bookings.filter(b => b.created_at.startsWith(date)).length
  ...
  const revenueData = Array.from({ length: 6 }, (_, i) => {
    ...
    const revenue = bookings
      .filter(b => b.payment_status === 'paid' && b.created_at.startsWith(format(date, 'yyyy-MM')))
      .reduce((sum, b) => sum + (b.total || 0), 0)
    return { month, revenue }
  })

  // Top vehicles
  const vehicleBookingCounts: Record<string, { name: string; count: number }> = {}
  bookings.forEach(b => {
    if (!vehicleBookingCounts[b.vehicle_id]) vehicleBookingCounts[b.vehicle_id] = { name: b.vehicle_id, count: 0 }
    vehicleBookingCounts[b.vehicle_id].count++
  })
  const topVehicles = Object.values(vehicleBookingCounts).sort((a, b) => b.count - a.count).slice(0, 5)
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Overview of St Michael Car Rentals operations</p>
      </div>
              ...
              {bookings.slice(0, 6).map(b => (
                <div key={b.id} className="flex items-center justify-between p-2 hover:bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <div className="text-xs text-gray-400 font-mono">{b.booking_reference}</div>
                  </div>
